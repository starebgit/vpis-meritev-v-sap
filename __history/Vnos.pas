...
uses
  ...
  SAPFunctionsOCX_TLB, Vcl.OleCtrls, SAPLogonCtrl_TLB, VpisKar, single, KonTocke, VpisSkup;
...
type
  ...
  private
    tabl,tabs,tabk,tabn,tabv,tabc : variant ;
...
begin
   ...
   begin
      ...
      begin
        ...
        funct.exports('PLANT').value := '0401'  ;
        funct.exports('CREAT_DAT').value := StrTodate(edit2.Text)   ;
      ...
      begin
        ...
        tx := edit4.Text ;
        ll := length(tx) ;
        //showMessage(IntTostr(tabc.rowcount)) ;
        for j := 1 to tabc.rowCount do
        begin
          //ShowMessage(tabc.value[j,1]) ;
          if tx = copy(tabc.value[j,12],1,ll) then
            lista.Add(tabc.value[j,1] + '  ' + tabc.value[j,12] + Format('%8d',[j])) ;
        end;
        if tabc.rowcount = 0 then label6.Caption := 'Nisem našel nobene kontrolne šarže' ;
      end ;
...
//  mt := tabc.value[ii+1,10] ;
  ...
  begin
    ...
    j := StrToInt(stx) ;
    mt := tabc.value[j,10] ;
    //ShowMessage(sarzeIzbor.Items[ii]) ;
    edit1.Text := Getmat(mt,'O') ;
    edit3.Text := tabc.value[j,13] ;
  end;
 // edit1.Text := Getmat(funct1.IMPORTS('GENERAL_DATA').VALUE(67),'O') ; ;
   //   if edit3.Text = '' then edit3.Text := funct1.IMPORTS('GENERAL_DATA').VALUE(88) ;
  ...
  begin
    ...
    //  funct1 := sapFunctions1.add('BAPI_INSPLOT_GETSTATUS');
      funct1.exports('NUMBER').value := srz ;
      funct1.exports('LANGUAGE').value[1] := 5  ;
      funct1.call ;


     // tabm := funct1.imports('GENERAL_DATA');
     tabm := funct1.TABLES.ITEM('SYSTEM_STATUS');
      //tabm := funct1.imports('USAGE_DECISION_DATA');
     // SHowMessage(tabm.value[1,1] + ' ' + tabm.value[1,2] + ' ' + tabm.value[1,3])   ;
  //    SHowMessage(intTostr(tabm.rowcount)) ;
      storno := false ;
      ...
      //SHowMessage(tabm.value[j,1] + ' ' + tabm.value[j,2] + ' ' + tabm.value[j,3]) ;

      //SHowMessage(tabm.value[7] + ' ' + tabm.value[8] + ' ' + tabm.value[9]) ;
      if storno then
      begin
...
begin
   ...
   begin
     ...
     begin
        ...
        funct.exports('INSPPOINTDATA').value(16) := date;
        funct.exports('INSPPOINTDATA').value(17) := time ;
        funct.exports('INSPPOINTDATA').value(18) := '3' ;
        funct.exports('INSPPOINTDATA').value(19) := '0401' ;
        funct.exports('INSPPOINTDATA').value(20) := 'A/R' ;
        funct.exports('INSPPOINTDATA').value(21) := 'A/R' ;
        funct.exports('INSPPOINTDATA').value(22) := dbred ;
         // funct.exports('INSPPOINTDATA').value(23) := 'Opomba' ;

        izap := FSqlTable.ZapisVzr(srz,opr,nazivp,dbred) ;
        ...
        begin
          ...
          Funct.Tables.Item('CHAR_RESULTS').Value[kk,25] := pb^.opom;
          ...
          begin
            ...
            if stip = '02' then
               ...
               Funct.Tables.Item('SINGLE_RESULTS').Value[kk,10] := pb^.merit ;
          end else
          begin
           ...
           Funct.Tables.Item('SAMPLE_RESULTS').Value[kk,17] := pb^.merit ;


            Funct.Tables.Item('CHAR_RESULTS').Value[kk,4] := 'X' ;
            Funct.Tables.Item('CHAR_RESULTS').Value[kk,5] := 'X' ;
           // Funct.Tables.Item('CHAR_RESULTS').Value[kk,6] := 'X' ;
            Funct.Tables.Item('CHAR_RESULTS').Value[kk,8] := dbred;
            Funct.Tables.Item('CHAR_RESULTS').Value[kk,15] := pb^.merit ;
            Funct.Tables.Item('CHAR_RESULTS').Value[kk,25] := pb^.opom ;
           // Funct.Tables.Item('CHAR_RESULTS').Value[kk,12] := '0' ;
          end;
        ...
        if not funct1.call then showMessage(Funct1.exception)
                           else FsqlTable.prenos(izap)
        end;
...
// Connection.System            := 'E01';
 ...
 // Connection.ApplicationServer := '10.0.2.159';
  Connection.ApplicationServer := '10.0.2.200';
//  Connection.SystemNumber      := '00';
 Connection.SystemNumber      := '20';
  //Connection.Password          := 'bongetsap';
   Connection.Password          := 'bon123sap';
   // Connection.Password          := 'rsg_rfc_1';
  Connection.Language          := 'sl'
end ;
